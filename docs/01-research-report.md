...
Markus стреляет в реальное узкое окно. Из 30+ продуктов в категориях AI second-brain, markdown-PKM и AI-journaling только два consumer-продукта (Recall и Capacities, оба в апреле 2026) имеют remote MCP коннектор, и ни один не комбинирует GitHub-as-storage с multi-assistant MCP для не-разработчиков.

Архитектура Markus технически крепкая и защищаемая. Главные риски не технические:
- Название Markus сильно занято в SEO и риск отказа в USPTO как фамилия
...
Differentiation сводится к матрице 4 свойств. Markus единственный кандидат который может закрыть все четыре:
...
**Урок: directory это маркетинг, custom remote MCP это выживание.** Markus должен работать как custom remote connector с дня один для платных Claude/ChatGPT/Perplexity юзеров вне зависимости от directory статуса.
...
Markus в $10-15 читается как native. Ниже $10 - commodity. Выше $20 нужен hardware или enterprise positioning.
...
Митигации stack:
- Frame Markus как **MCP server, не Claude connector**
- Multi-vendor (Claude + ChatGPT + Perplexity) с дня один
- BYOK для свободного hopping providers
- **Данные в GitHub репо юзера - даже если Markus завтра умрет, у юзера полный markdown vault** (это лучшая защита и должна быть lead marketing claim)
...
Преимущество Markus: **structured layer derivable, не authored**. Хранить atomic notes как markdown + YAML frontmatter в GitHub, derive graph и embeddings на Markus серверах, recompute on demand.
...
- Markus AI for events (`dearmarkus.ai`, `agentmarkus.com`) - press в Q1 2026 pilots
...
USPTO 2(e)(4) refusal risk: "primarily merely a surname" так как Markus - common given name. Stylized logo или compound (`MARKUS AI`, `MARKUS BRAIN`) substantially improves registrability.

Premium домены `markus.app`, `markus.ai`, `markus.com` почти точно taken на 5-6 значные суммы.
...
- **"Markus - Auto Second Brain"** ⭐ winner
